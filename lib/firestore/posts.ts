...
import {
  ...
  limit as fsLimit, getDoc, getDocs, orderBy, query,
...
export interface Post {
  ...
  media_urls: string[]; visibility: PostVisibility; community_id?: string | null;
  post_type?: PostType | null; is_visible?: boolean | null; hashtags?: string[];
  poll?: import("@/lib/firestore/polls").PollData | null;
  like_count: number; comment_count: number; share_count: number;
  created_at: string; updated_at: string;
  user: ProfileRow | null; community: CommunityRow | null;
  is_liked?: boolean; is_saved?: boolean; is_owned?: boolean;
}

export interface PostFilters {
  limit?: number; communitySlug?: string; communityIds?: string[];
  username?: string; userId?: string; hashtag?: string;
  visibility?: PostVisibility; sortBy?: "newest" | "popular" | "trending";
  language?: string | null;
  cursor?: { lastDocId?: string } | null;
}

export interface PaginatedPosts {
  posts: Post[]; total: number; hasMore: boolean; nextCursor: PostFilters["cursor"];
}

export type CreatePostData = {
  title?: string; content: string; media?: MediaItem[];
  community_id?: string; visibility: PostVisibility;
};

export type UpdatePostData = {
  title?: string | null; content?: string | null; media_urls?: string[] | null;
  visibility?: PostVisibility | null; community_id?: string | null;
  is_visible?: boolean | null; post_type?: PostType | null;
};
...
const POST_MEDIA_TYPES: ReadonlySet<MediaType> = new Set(["image", "video", "gif"]);
...
function normalizePostType(p: { post_type?: any; media_urls?: any }): PostType {
  const t = p?.post_type;
  if (t === "text" || t === "image" || t === "video" || t === "mixed" || t === "poll") return t;
...
function docToPost(id: string, d: any, extras?: Partial<Post>): Post {
  return {
    id, user_id: d.user_id, title: d.title ?? null, content: d.content ?? "",
    media_urls: Array.isArray(d.media_urls) ? d.media_urls : [],
    visibility: normalizeVisibility(d.visibility), community_id: d.community_id ?? null,
    post_type: normalizePostType(d), is_visible: typeof d.is_visible === "boolean" ? d.is_visible : true,
    hashtags: Array.isArray(d.hashtags) ? d.hashtags : [], poll: d.poll ?? null,
    ...
    user: d.user ?? null, community: d.community ?? null, ...extras,
  };
}

async function resolveCommunityIdFromSlug(slug: string): Promise<string | null> {
...
async function resolveUserIdFromUsername(username: string): Promise<string | null> {
  const raw = username.trim(); if (!raw) return null;
...
async function getProfileSnapshot(uid: string): Promise<ProfileRow | null> {
  ...
  return { id: uid, username: d.username ?? "", full_name: d.full_name ?? null, avatar_url: d.avatar_url ?? null };
}

async function getCommunitySnapshot(communityId: string): Promise<CommunityRow | null> {
  const snap = await getDoc(doc(db, "communities", communityId));
  if (!snap.exists()) return null;
  const d = snap.data() as any;
  return { id: communityId, name: d.name ?? "", slug: d.slug ?? "", avatar_url: d.avatar_url ?? d.image_url ?? null };
}
...
function isRemoteUrl(u: string): boolean { return /^https?:\/\//i.test(u); }
...
async function uploadMediaForPost(userId: string, media: MediaItem[] | undefined): Promise<string[]> {
  if (!media?.length) return [];
  const urls: string[] = [];

  for (const item of media) {
    if (!POST_MEDIA_TYPES.has(item.type)) continue;
    if (isRemoteUrl(item.uri)) { urls.push(item.uri); continue; }

    const fallbackExt = item.type === "video" ? "mp4" : item.type === "gif" ? "gif" : "jpg";
...
async function fetchMyLikeSaveFlags(uid: string, postIds: string[]) {
  if (!uid || !postIds.length) return { liked: new Set<string>(), saved: new Set<string>() };
  const chunks = chunk(postIds, 10);
  const liked = new Set<string>();
  const saved = new Set<string>();
  await Promise.all([
    (async () => { for (const c of chunks) { const q1 = query(LIKES, where("user_id", "==", uid), where("post_id", "in", c)); const s1 = await getDocs(q1); s1.docs.forEach((d) => liked.add((d.data() as any).post_id)); } })(),
    (async () => { for (const c of chunks) { const q2 = query(SAVES, where("user_id", "==", uid), where("post_id", "in", c)); const s2 = await getDocs(q2); s2.docs.forEach((d) => saved.add((d.data() as any).post_id)); } })(),
  ]);
  return { liked, saved };
}
...
export async function getPosts(filters: PostFilters = {}): Promise<PaginatedPosts> {
  const { limit = 20, communitySlug, communityIds, username, userId, hashtag, visibility, sortBy = "newest", language = null, cursor = null } = filters;
  ...
  if (username && !resolvedUserId) resolvedUserId = await resolveUserIdFromUsername(username);

  const wheres: any[] = [];
  if (resolvedCommunityIds.length) wheres.push(where("community_id", "in", resolvedCommunityIds.slice(0, 10)));
  if (resolvedUserId) wheres.push(where("user_id", "==", resolvedUserId));
  if (visibility) wheres.push(where("visibility", "==", visibility));
  if (hashtag) wheres.push(where("hashtags", "array-contains", hashtag.toLowerCase().replace(/^#/, "")));
  // ✅ Language filter — only apply when user has a non-English preference set
  if (language && language !== "en") wheres.push(where("language", "==", language));

  const extra: any[] = [];
  if (sortBy === "trending") {
    const weekAgo = Timestamp.fromDate(new Date(Date.now() - 7 * 86400000));
    extra.push(where("created_at_ts", ">=", weekAgo), orderBy("like_count", "desc"), orderBy("created_at_ts", "desc"));
  ...
  const raw = snap.docs.map((d) => docToPost(d.id, d.data())).filter((p) => p.is_visible !== false).slice(0, limit);
  const ids = raw.map((p) => p.id);
  const { liked, saved } = viewerId ? await fetchMyLikeSaveFlags(viewerId, ids) : { liked: new Set<string>(), saved: new Set<string>() };

  const posts = raw.map((p) => docToPost(p.id, p, {
  ...
  const nextCursor: PostFilters["cursor"] = last ? { lastDocId: last.id } : null;
  ...
  if (!cursor) {
    try {
      ...
      if (boostedIds.length > 0) {
        ...
        return { posts: [...boosted, ...normal], total: -1, hasMore: snap.docs.length >= limit, nextCursor };
      }
    } catch { /* boost fetch failed */ }
  }
...
export async function getPostById(id: string): Promise<Post | null> {
  const clean = id?.trim(); if (!clean) return null;
  const snap = await getDoc(doc(db, "posts", clean));
  if (!snap.exists()) return null;
  const d = snap.data() as any;
  if (d.is_visible === false) return null;
  const viewerId = auth.currentUser?.uid ?? "";
  let is_liked = false, is_saved = false;
  if (viewerId) {
    const [likeSnap, saveSnap] = await Promise.all([
      getDocs(query(LIKES, where("user_id", "==", viewerId), where("post_id", "==", clean), fsLimit(1))),
      getDocs(query(SAVES, where("user_id", "==", viewerId), where("post_id", "==", clean), fsLimit(1))),
    ]);
    is_liked = !likeSnap.empty; is_saved = !saveSnap.empty;
  }
  return docToPost(snap.id, d, { is_liked, is_saved, is_owned: viewerId ? d.user_id === viewerId : false });
}
...
export async function createPost(postData: CreatePostData): Promise<Post | null> {
  ...
  const refDoc = await addDoc(POSTS, {
    user_id: uid, title: postData.title ?? null, content: postData.content,
    community_id: postData.community_id ?? null, visibility: postData.visibility,
    media_urls: urls, post_type, is_visible: true,
    language: detectedLanguage ?? "en",
    like_count: 0, comment_count: 0, share_count: 0,
    user: profileSnap, community: communitySnap,
    created_at: now, updated_at: now,
    created_at_ts: serverTimestamp(), updated_at_ts: serverTimestamp(),
  });
  const createdSnap = await getDoc(refDoc);
  if (!createdSnap.exists()) return null;
  return docToPost(createdSnap.id, createdSnap.data(), { is_owned: true, is_liked: false, is_saved: false });
}
...
export async function updatePost(postId: string, patch: UpdatePostData): Promise<Post | null> {
  const viewer = auth.currentUser;
  if (!viewer) throw new Error("User not authenticated");
  const uid = viewer.uid;
  ...
  if (patch.community_id !== undefined) communitySnap = patch.community_id ? await getCommunitySnapshot(patch.community_id) : null;
  const now = new Date().toISOString();
  await updateDoc(refDoc, { ...patch, ...(communitySnap !== undefined ? { community: communitySnap } : {}), updated_at: now, updated_at_ts: serverTimestamp() });
