...
/**
 * One page of the swipeable viewer — a single post's full card, actions,
 * and collapsible comments. Mirrors app/post/[id].tsx's layout, but as
 * a standalone component that takes postId as a prop instead of reading
 * route params, and only loads comments once the user taps to expand them.
 */
function PostViewerPage({
  ...
  // ✅ Comments only fetched once the user expands them on this page —
  ...
  // Only check repost status once this page becomes active — avoids
  // firing a read for every page mounted in the FlatList window.
  React.useEffect(() => {
    ...
    <View style={{ width: SCREEN_W, flex: 1 }}>
      ...
      >
        <ScrollView
          ...
          // Swipe gesture for the parent pager needs horizontal drags to
          // win on this ScrollView's edges — vertical-only scroll here
          // keeps horizontal swipes free for FlatList paging.
