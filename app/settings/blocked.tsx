...
interface BlockedUser {
  id: string; blocked_id: string; created_at: string;
  profile: { username: string; full_name?: string; avatar_url?: string };
}
interface MutedUser {
  id: string; muted_id: string; created_at: string;
  profile: { username: string; full_name?: string; avatar_url?: string };
}
...
export default function BlockedAccountsScreen() {
  ...
  const tabCounts = useMemo(() => ({ blocked: blockedUsers.length, muted: mutedUsers.length }), [blockedUsers.length, mutedUsers.length]);

  const renderUserRow = (item: BlockedUser | MutedUser, mode: "blocked" | "muted") => {
    ...
    const actionPending = mode === "blocked" ? unblockUser.isPending : unmuteUser.isPending;

    const action = () => {
      if (mode === "blocked") {
        Alert.alert("Unblock User", `Unblock @${handle}?`, [
          { text: "Cancel", style: "cancel" },
          { text: "Unblock", style: "destructive", onPress: () => unblockUser.mutate((item as BlockedUser).blocked_id) },
        ]);
      } else {
        Alert.alert("Unmute User", `Unmute @${handle}?`, [
          { text: "Cancel", style: "cancel" },
          { text: "Unmute", onPress: () => unmuteUser.mutate((item as MutedUser).muted_id) },
        ]);
      }
    };
    ...
    return (
      <View style={[styles.userCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <View style={[styles.avatarCircle, { backgroundColor: colors.primary + "15", borderColor: colors.border }]}>
          <Ionicons name={mode === "blocked" ? "ban-outline" : "volume-mute-outline"} size={18} color={colors.primary} />
        </View>
        <View style={styles.userInfo}>
          <Text style={[styles.userName, { color: colors.text }]} numberOfLines={1}>{displayName}</Text>
          <Text style={[styles.userHandle, { color: colors.textSecondary }]} numberOfLines={1}>@{handle}</Text>
          <Text style={[styles.userMeta, { color: colors.textTertiary }]}>{dateLabel}</Text>
        </View>
        <TouchableOpacity
          style={[styles.actionBtn, { backgroundColor: mode === "blocked" ? colors.error : colors.primary }]}
          onPress={action}
          disabled={actionPending}
          activeOpacity={0.85}
        >
          {actionPending ? <ActivityIndicator size="small" color="#fff" /> : (
            <Text style={styles.actionText}>{mode === "blocked" ? "Unblock" : "Unmute"}</Text>
          )}
        </TouchableOpacity>
      </View>
    );
  ...
  const Empty = ({ mode }: { mode: "blocked" | "muted" }) => (
    <View style={[styles.emptyWrap, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <View style={[styles.emptyIcon, { backgroundColor: colors.primary + "15" }]}>
        <Ionicons name={mode === "blocked" ? "ban-outline" : "volume-mute-outline"} size={30} color={colors.primary} />
      </View>
  ...
  const content = (
    <SafeAreaView style={{ flex: 1, backgroundColor: "transparent" }} edges={["top", "left", "right"]}>
      ...
      <View style={styles.header}>
        <Text style={[styles.title, { color: colors.text }]}>Blocked & Muted</Text>
          ...
          {(["blocked", "muted"] as const).map((tab) => (
            ...
            >
              ...
              <Text style={[styles.tabText, { color: activeTab === tab ? "#fff" : colors.textSecondary }]}>
                {tab === "blocked" ? "Blocked" : "Muted"}{tabCounts[tab] ? ` (${tabCounts[tab]})` : ""}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
      ...
      <FlatList
        data={(activeTab === "blocked" ? blockedUsers : mutedUsers) as any[]}
        keyExtractor={(i) => i.id}
        ...
        ListFooterComponent={
          <View style={[styles.infoCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[styles.infoTitle, { color: colors.text }]}>Quick difference</Text>
              <Text style={[styles.infoText, { color: colors.textSecondary }]}>
                <Text style={{ fontWeight: "900", color: colors.text }}>Blocked</Text>: they can't see or interact with you.{"\n"}
                <Text style={{ fontWeight: "900", color: colors.text }}>Muted</Text>: they can see you, but you won't see their content.
              </Text>
            </View>
          </View>
        }
...
const styles = StyleSheet.create({
  ...
  tabPill: { marginTop: 12, borderRadius: 999, padding: 4, flexDirection: "row", gap: 6 },
  tab: { flex: 1, borderRadius: 999, paddingVertical: 10, flexDirection: "row", gap: 8, alignItems: "center", justifyContent: "center" },
  tabText: { fontSize: 13, fontWeight: "700" },
  list: { paddingHorizontal: 16, paddingTop: 6, paddingBottom: 8, gap: 10 },
  userCard: { borderRadius: 18, padding: 14, flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1 },
  ...
  actionBtn: { paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, minWidth: 88, alignItems: "center", justifyContent: "center" },
  actionText: { color: "#fff", fontSize: 13, fontWeight: "800" },
  emptyWrap: { marginTop: 20, borderRadius: 18, padding: 18, alignItems: "center", borderWidth: 1 },
  emptyIcon: { width: 54, height: 54, borderRadius: 27, alignItems: "center", justifyContent: "center", marginBottom: 10 },
  emptyTitle: { fontSize: 14, fontWeight: "900" },
  emptySub: { fontSize: 12, textAlign: "center", marginTop: 6, lineHeight: 18 },
  infoCard: { flexDirection: "row", gap: 10, margin: 16, marginTop: 8, padding: 14, borderRadius: 18, borderWidth: 1 },
