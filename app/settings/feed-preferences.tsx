// app/settings/feed-preferences.tsx — UPDATED ✅
// ✅ NSFW toggle locked for users under 18
...
export default function FeedPreferencesScreen() {
  ...
  // ✅ Check if NSFW is locked due to age
  const nsfwLocked =
    (settings as any)?.nsfw_locked === true ||
    (profile as any)?.age_group === "teen" ||
    (profile as any)?.age_group === "under_13";
      ...
      rows: [
        {
          ...
          // ✅ Show locked description for under 18
          description: nsfwLocked
            ? "Not available for users under 18"
            : "Display content marked as not safe for work",
          icon: nsfwLocked ? "lock-closed-outline" : "warning-outline",
          toggle: true,
          toggleValue: nsfwLocked ? false : !!settings?.preferences?.show_nsfw,
          locked: nsfwLocked,
          onToggle: (v) => {
            if (nsfwLocked) {
              Alert.alert(
                "Restricted",
                "This setting is not available for users under 18.",
              );
              return;
            }
            handleToggle("show_nsfw", v);
          },
        },
        {
    ...
    >
      ...
      >
        <Ionicons
          name={item.icon}
          size={18}
          color={item.locked ? colors.textTertiary : colors.primary}
        />
      </View>
      <View style={styles.rowText}>
        <Text
          style={[
            styles.rowTitle,
            { color: item.locked ? colors.textSecondary : colors.text },
          ]}
        >
          {item.title}
        </Text>
